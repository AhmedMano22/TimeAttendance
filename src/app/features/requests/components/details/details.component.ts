...
export class DetailsComponent {
  ...
  ngOnInit() {
    ...
    if (this.mode == "view" || this.mode == "edit") {
      this.apiSer.getRequestByID(this.id).subscribe(
        (res: any) => {
          console.log(res);
          this.requestinfo = {
            Id: res[0].Id,
          ...
          this.picPreview = this.requestinfo.Logo64;
  ...
  onFileSelected(event: any) {
    ...
    reader.onload = (e) => {
      ...
      this.requestinfo.Logo64 = reader.result as string;
  ...
  onSubmit(): void {
    ...
    } else if (this.requestinfo.Logo64) {
      const blob = this.base64ToBlob(this.requestinfo.Logo64, "image/png");
      const file = this.createFileFromBlob(blob, "logo.png");
      formData.append("Logo", file);
    }

    this.apiSer.UpdateRequest(this.id, formData).subscribe(
      (res: any) => {
        console.log("response", res);
        if (res.IsSuccessStatusCode) {
          this.translate
            .get("updaterequestsweetAlert")
