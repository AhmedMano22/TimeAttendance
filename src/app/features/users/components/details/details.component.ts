...
export class DetailsComponent {
  ...
  ngOnInit() {
    ...
    this.apiSer.getPendingOwnerByID(this.id).subscribe(
      (res: any) => {
        console.log(res);
        this.PendingOwnerinfo = {
          Id: res[0].Id,
          Name: res[0].Name,
          Phone: res[0].Phone,
          Email: res[0].Email,
  ...
  onSubmit(modal: any): void {
    ...
    this.apiSer.Reject(this.id, this.form.value).subscribe((res: any) => {
      console.log("response", res);
      if (res.Status == "Reject") {
        modal.dismiss();
        this.translate
          .get("rejectsweetAlert")
          .subscribe((translations: any) => {
            Swal.fire({
              ...
              if (result.isConfirmed) {
                this.router.navigate(["/users/Pending-Owners"]);
              }
            });
          });
      }
